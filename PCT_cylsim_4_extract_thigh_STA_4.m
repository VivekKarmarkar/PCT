...
box_markers(2).name = "Anterior High";
box_markers(2).idx_bool = false(NumMarkers, 1);

box_markers(3).name = "Lateral Low";
box_markers(3).idx_bool = false(NumMarkers, 1);

box_markers(4).name = "Lateral High";
...
for j=1:NumMarkers
    ...
    box_val = eval_marker_four(theta(j), h(j));
...
x_offset_total = nan(NumTrials, N);
y_offset_total = nan(NumTrials, N);
z_offset_total = nan(NumTrials, N);
for k=1:N
    box_idx_current = k;
    input_data.box_idx_current = box_idx_current;

    vector_normalized_mean = generate_box_marker_data_four(input_data);
...
norm_offset_cm_4 = sqrt(x_offset_cm.^2 + y_offset_cm.^2 + z_offset_cm.^2);
...
figure
plot(norm_offset_cm_4/10, 'r', 'LineWidth', 1.5, 'DisplayName', 'x CM offset');
hold on;
for j=1:4
...
save('box_markers_gait_4.mat', 'box_markers_gait');
...
function vector_normalized_mean = generate_box_marker_data_four(input_data)
