...
angleOne_new_offsetSVDLS = nan(NumTrials, 1);
angleTwo_new_offsetSVDLS = nan(NumTrials, 1);
angleThree_new_offsetSVDLS = nan(NumTrials, 1);
...
CMnorm_new_offsetSVDLS = nan(NumTrials, 1);
...
global_offset_data_SVDLS = nan(NumTrials, N, 1);
...
global_offset_data_SVDLS_AL = nan(NumTrials, Num_AL_Thigh, 1);
...
for m=1:NumTrials
    ...
    visualize_inputSVDLS = visualize_input;
    ...
    visualize_inputSVDLS.mass_distribution_SVDLS = mass_distribution_noisy;
    [T_SVDLS, R_SVDLS] = pose_estimate_SVD_LS(visualize_inputSVDLS);
    ...
    visualize_inputSVDLS.inertial_properties_SVDLS = inertial_properties_SVDLS;
    visualize_inputSVDLS = compute_Final_localOffset_SVDLS(visualize_inputSVDLS);
    ...
    anatomical_landmarks_thigh_data_SVDLS = compute_anatomical_landmarks_thigh_data(cylinder_data, reference_pose, optimized_pose_SVDLS);
    visualize_inputSVDLS.anatomical_landmarks_thigh_data = anatomical_landmarks_thigh_data_SVDLS;
    ...
    for j=1:N
        ...
        global_offset_data_SVDLS(m, j, 1) = visualize_inputSVDLS.mass_distribution_SVDLS(j).global_offset;
    ...
    for j=1:Num_AL_Thigh
        ...
        global_offset_data_SVDLS_AL(m, j, 1) = visualize_inputSVDLS.anatomical_landmarks_thigh_data(j).global_offset;
    end
    ...
    H = horzcat(global_offset_data(m, :)', global_offset_data2D(m, :)', global_offset_data2D_constrained_new_1(m, :)', global_offset_data2D_constrained_new_2(m, :)', global_offset_data2D_constrained_new_3(m, :)', global_offset_data_noisy(m, :)', global_offset_data_perturbed(m, :)', global_offset_data_reflected(m, :)', global_offset_data_hybrid(m, :)', global_offset_data_SVDLS(m, :)');
    [H_min, I_min] = min(H,[],2);
    ...
    current_PCT_performanceSVDLS = evaluate_PCT_SVDLS(inertial_properties_SVDLS_offset, inertial_properties_offset);
    angleOne_new_offsetSVDLS(m) = current_PCT_performanceSVDLS.angle.newList(1);
    angleTwo_new_offsetSVDLS(m) = current_PCT_performanceSVDLS.angle.newList(2);
    angleThree_new_offsetSVDLS(m) = current_PCT_performanceSVDLS.angle.newList(3);
    CMnorm_new_offsetSVDLS(m) = current_PCT_performanceSVDLS.CM_distance.newNorm;
...
plot(angleOne_new_offsetSVDLS, 'k', 'DisplayName', 'SVDLS');
grid;
legend;
...
plot(angleTwo_new_offsetSVDLS, 'k', 'DisplayName', 'SVDLS');
grid;
...
plot(CMnorm_new_offsetSVDLS, 'k', 'DisplayName', 'SVDLS');
grid;
...
global_offset_data_SVDLS_max = nan(NumTrials,1);
...
std_global_offset_data_SVDLS = nan(NumTrials, 1);
...
for k=1:NumTrials
    ...
    global_offset_data_SVDLS_max(k,1) = max(global_offset_data_SVDLS(k,:));
    ...
    std_global_offset_data_SVDLS(k,1) = std(global_offset_data_SVDLS(k,:));
end
...
subplot(3,4,4);
histogram(global_offset_data_SVDLS_max, nbins, 'DisplayName', 'Histogram');
hold on;
xline(nanmean(global_offset_data_SVDLS_max), 'k', 'LineWidth', 1.5, 'DisplayName', 'Mean');
xline(nanmedian(global_offset_data_SVDLS_max), 'k--', 'LineWidth', 1.5, 'DisplayName', 'Median');
legend;
title("SVD LS");
...
subplot(3,4,4);
histogram(CMnorm_new_offsetSVDLS, nbins, 'DisplayName', 'Histogram');
hold on;
xline(nanmean(CMnorm_new_offsetSVDLS), 'k', 'LineWidth', 1.5, 'DisplayName', 'Mean');
xline(nanmedian(CMnorm_new_offsetSVDLS), 'k--', 'LineWidth', 1.5, 'DisplayName', 'Median');
legend;
title("SVD LS");
...
subplot(5,2,9);
histogram(global_offset_data_SVDLS_max(plane_bool), nbins, 'DisplayName', 'Histogram');
hold on;
xline(nanmean(global_offset_data_SVDLS_max(plane_bool)), 'k', 'LineWidth', 1.5, 'DisplayName', 'Mean');
xline(nanmedian(global_offset_data_SVDLS_max(plane_bool)), 'k--', 'LineWidth', 1.5, 'DisplayName', 'Median');
legend;
title("SVD LS: Planar Distributions");
xlabel("Error (cm)");
ylabel("Frequency");
...
xline(nanmedian(global_offset_data_SVDLS_max(~plane_bool)), 'k--', 'LineWidth', 1.5, 'DisplayName', 'Median');
legend;
title("SVD LS: Non-Planar Distributions");
xlabel("Error (cm)");
...
subplot(5,2,9);
histogram(CMnorm_new_offsetSVDLS(plane_bool), nbins, 'DisplayName', 'Histogram');
hold on;
xline(nanmean(CMnorm_new_offsetSVDLS(plane_bool)), 'k', 'LineWidth', 1.5, 'DisplayName', 'Mean');
xline(nanmedian(CMnorm_new_offsetSVDLS(plane_bool)), 'k--', 'LineWidth', 1.5, 'DisplayName', 'Median');
legend;
title("SVD LS: Planar Distributions");
xlabel("Error (cm)");
ylabel("Frequency");
...
xlim_min = min([0; min(CMnorm_new_offset); min(CMnorm_new_offset_reflected); min(CMnorm_new_offsetPerturbed); min(CMnorm_new_offsetSVDLS)]);
xlim_max = max([max(CMnorm_new_offset); max(CMnorm_new_offset_reflected); max(CMnorm_new_offsetPerturbed); max(CMnorm_new_offsetSVDLS)]) + 1;
...
for j=1:Num_AL_Thigh
    ...
    plot(global_offset_data_SVDLS_AL(:, j), 'k', 'DisplayName', 'SVD LS');
...
for j=1:Num_AL_Thigh
    ...
    histogram(global_offset_data_SVDLS_AL(:, j), nbins, 'FaceColor', 'k', 'FaceAlpha', 0.1, 'EdgeColor', 'None', 'DisplayName', 'SVD LS');
    current_title_str = visualize_inputPerturbed.anatomical_landmarks_thigh_data(j).name;
    title(current_title_str);
    if j==2
        legend;
...
xlim_std_min = min([0; min(std_global_offset_data); min(std_global_offset_data_reflected); min(std_global_offset_data_perturbed); min(std_global_offset_data_SVDLS)]);
xlim_std_max = max([max(std_global_offset_data); max(std_global_offset_data_reflected); max(std_global_offset_data_perturbed); max(std_global_offset_data_SVDLS)]);
...
subplot(5,1,5);
histogram(std_global_offset_data_SVDLS, nbins, 'DisplayName', 'Histogram');
hold on;
xline(nanmean(std_global_offset_data_SVDLS), 'k', 'LineWidth', 1.5, 'DisplayName', 'Mean');
xline(nanmedian(std_global_offset_data_SVDLS), 'k--', 'LineWidth', 1.5, 'DisplayName', 'Median');
legend;
xlim([xlim_std_min, xlim_std_max]);
title("SVD LS");
xlabel("Error (cm)");
...
function visualize_input = compute_Final_localOffset_SVDLS(visualize_input)
    T = visualize_input.inertial_properties_SVDLS.T;
    R = visualize_input.inertial_properties_SVDLS.R;
    N = visualize_input.N;
    for k=1:N
        current_global_vector = visualize_input.mass_distribution_SVDLS(k).vector;
        current_ref_vector = visualize_input.mass_distribution_reference(k).local_displacement;
        original_global_vector = visualize_input.mass_distribution_reference(k).vector;
        current_local_vector = global2local(T, R, current_global_vector);
        global_vector_reconstructed = local2global(T, R, current_ref_vector);
        global_offset = norm(global_vector_reconstructed - original_global_vector);
        visualize_input.mass_distribution_SVDLS(k).local_displacement = current_local_vector;
        visualize_input.mass_distribution_SVDLS(k).local_offset = norm(current_local_vector - current_ref_vector);
        visualize_input.mass_distribution_SVDLS(k).global_reconstructed = global_vector_reconstructed;
        visualize_input.mass_distribution_SVDLS(k).global_offset = global_offset;
    end
...
function [T_val, R_val] = pose_estimate_SVD_LS(visualize_input)
    N = visualize_input.N;
    
    global_vectors = struct;
    local_vectors = struct;
    for c=1:N
        global_vectors(c).vec = visualize_input.mass_distribution_noisy(c).vector;
        ...
        local_vectors(c).vec = visualize_input.mass_distribution_reference(c).local_displacement;
