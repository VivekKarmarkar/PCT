...
for j=1:NumConfigurations   
    ...
    mass_redistribution = compute_mass_redistribution(output_containers, input_size);
    scaling_factor_CM = compute_scaling_factor_CM(mass_redistribution, output_containers, input_size);
    output_containers = reprocess_some_algorithms(input_struct, output_containers, temp_containers, mass_redistribution, scaling_factor_CM);
...
function new_rotationMatrix = compute_rotation_given_translation(T, N, mass_distribution_noisy, mass_distribution_reference)
    D = 0;
    ...
    for k=1:N
        position_global = reshape(mass_distribution_noisy(k).vector, [3,1]);
        position_global_translated = position_global - T;
        position_local = reshape(mass_distribution_reference(k).local_displacement, [3,1]);
        A = leftquat(position_global_translated) - rightquat(position_local);
        D = D + transpose(A)*A;
        
        xdata(:,k) = position_local;
        ydata(:,k) = position_global;
    end
    
    [evec_D, eval_D] = eig(D);
    [~, I] = sort(diag(eval_D));
    q_star = evec_D(:,I(1));
    new_rotationMatrix = quat2rotm(transpose(q_star));
    
end

function leftquat_matrix = leftquat(inputvec)
    v = inputvec;
    leftquat_matrix = [0 -v(1) -v(2) -v(3);
                       v(1) 0 -v(3) v(2);
                       v(2) v(3) 0 -v(1);
                       v(3) -v(2) v(1) 0];
end

function rightquat_matrix = rightquat(inputvec)
    v = inputvec;
    rightquat_matrix = [0 -v(1) -v(2) -v(3);
                       v(1) 0 v(3) -v(2);
                       v(2) -v(3) 0 v(1);
                       v(3) v(2) -v(1) 0];
end

function scaling_factor_CM = compute_scaling_factor_CM(m, output_containers, input_size)
    
    N = input_size.N;
    NumFrames = input_size.NumFrames;
    
    x_cm_PT_list = nan(NumFrames, 1);
    y_cm_PT_list = nan(NumFrames, 1);
    z_cm_PT_list = nan(NumFrames, 1);
    vector_cm_PT_list = nan(NumFrames, 3);
    
    x_cm_N_list = nan(NumFrames, 1);
    y_cm_N_list = nan(NumFrames, 1);
    z_cm_N_list = nan(NumFrames, 1);
    vector_cm_N_list = nan(NumFrames, 3);
    
    distance_threshold_cm_list = nan(NumFrames, 1);
    ...
    for j=1:NumFrames 
        ...
        for i=1:N
            current_noisy_position = reshape(output_containers.noisy_positions.PTNR(j, i, :), [3,1]);
            x_sum_PT = x_sum_PT + m(i)*current_noisy_position(1);
            y_sum_PT = y_sum_PT + m(i)*current_noisy_position(2);
            z_sum_PT = z_sum_PT + m(i)*current_noisy_position(3);
            m_sum_PT = m_sum_PT + m(i);
            
            x_sum_N = x_sum_N + current_noisy_position(1);
            y_sum_N = y_sum_N + current_noisy_position(2);
            z_sum_N = z_sum_N + current_noisy_position(3);
            m_sum_N = m_sum_N + 1.0;
        end
        
        x_cm_PT = x_sum_PT/m_sum_PT;
        y_cm_PT = y_sum_PT/m_sum_PT;
        z_cm_PT = z_sum_PT/m_sum_PT;
        x_cm_PT_list(j) = x_cm_PT;
        y_cm_PT_list(j) = y_cm_PT;
        z_cm_PT_list(j) = z_cm_PT;
        vector_cm_PT_list(j,:) = [x_cm_PT, y_cm_PT, z_cm_PT];
        
        x_cm_N = x_sum_N/m_sum_N;
        y_cm_N = y_sum_N/m_sum_N;
        z_cm_N = z_sum_N/m_sum_N;
        x_cm_N_list(j) = x_cm_N;
        y_cm_N_list(j) = y_cm_N;
        z_cm_N_list(j) = z_cm_N;
        vector_cm_N_list(j,:) = [x_cm_N, y_cm_N, z_cm_N];
        
        fraction_completed_gait_cycle = input_size.fraction_completed_gait_cycle(j);
        cm_dist_threshold = compute_cm_dist_threshold_STA(fraction_completed_gait_cycle);
        distance_threshold_cm_list(j) = cm_dist_threshold;
        
    end
    
    scaling_factor_CM = 1;
    while scaling_factor_CM >= 0.1
        vector_cm_PT_constrained_list = (1 - scaling_factor_CM)*vector_cm_N_list + scaling_factor_CM*vector_cm_PT_list;
        vector_cm_REL_constrained_list = vector_cm_PT_constrained_list - vector_cm_N_list;
        norm_cm_REL_constrained_list = vecnorm(vector_cm_REL_constrained_list,2,2);
        if all(norm_cm_REL_constrained_list < distance_threshold_cm_list)
            break;
        end
        scaling_factor_CM = scaling_factor_CM - 0.1;
    end
...
function mass_redistribution = compute_mass_redistribution(output_containers, input_size)
    N = input_size.N;
    funcAll = @(m)nLsysMDall(m, output_containers, input_size);
    m0 = ones(N,1);
    options = optimoptions('fsolve','Algorithm','levenberg-marquardt','MaxFunctionEvaluations', 10000, 'FunctionTolerance', 1.0);
    m_star = fsolve(funcAll, m0, options);
    mass_redistribution = m_star;
end
...
function [output_containers, temp_containers] = process_all_algorithms(input_struct, output_containers)
    ...
    for j=1:NumFrames    
        ...
        algorithm_container.PTNR.m0 = ones(N,1);
        ...
        temp_containers(j).algorithm_containerPT = algorithm_container.PTNR;
        ...
        for k=1:N
            ...
            output_containers.noisy_positions.PTNR(j, k, :) = algorithm_container.PTNR.mass_distribution_noisy(k).vector;
        end
        ...
        output_containers.inertiaTensor.PTNR(j, :, :) = algorithm_container.PTNR.inertiaTensor_final;
        
    end
end

function output_containers = reprocess_some_algorithms(input_struct, output_containers, temp_containers, mass_redistribution, scaling_factor_CM)
    input_size = input_struct.input_size;
    input_settings = input_struct.input_settings;
    input_data = input_struct.input_data;
    ...
    for j=1:NumFrames    
        ...
        [inertial_properties_reference, ~] = compute_inertial_properties(mass_distribution_reference, N);
        [inertial_properties_noisy, ~] = compute_inertial_properties(mass_distribution_noisy, N);
        inertial_properties_offset = calculate_offset(inertial_properties_reference, inertial_properties_noisy);
        if initial_axes_reversal
            inertial_properties_noisy_reversed = axis_reversal_original(inertial_properties_noisy, inertial_properties_offset);
            inertial_properties_noisy = inertial_properties_noisy_reversed;
        ...
        algorithm_container.PTNR = temp_containers(j).algorithm_containerPT;
        algorithm_container.PTNR.mass_redistribution = mass_redistribution;
        algorithm_container.PTNR.scaling_factor_CM = scaling_factor_CM;
        algorithm_container.PTNR.reflection_setting = "NR";
        algorithm_container.PTNR = Perturbation_Theory_reprocessing(algorithm_container.PTNR);
        
        algorithm_container.PTUR = temp_containers(j).algorithm_containerPT;
        algorithm_container.PTUR.mass_redistribution = mass_redistribution;
        algorithm_container.PTUR.scaling_factor_CM = scaling_factor_CM;
        algorithm_container.PTUR.reflection_setting = "UR";
        algorithm_container.PTUR = Perturbation_Theory_reprocessing(algorithm_container.PTUR);
        
        algorithm_container.PTLR = temp_containers(j).algorithm_containerPT;
        algorithm_container.PTLR.mass_redistribution = mass_redistribution;
        algorithm_container.PTLR.scaling_factor_CM = scaling_factor_CM;
        algorithm_container.PTLR.reflection_setting = "LR";
        algorithm_container.PTLR = Perturbation_Theory_reprocessing(algorithm_container.PTLR);
...
function generate_output_plots(input_struct, output_containers)
    ...
    color_list = ['k', 'b', 'r', 'm'];
    alg_list = ["SVDLS", "PTNR", "PTUR"];

    path_img = "C:\Users\vkarmarkar\OneDrive - University of Iowa\Desktop\Research\Code\Test Code\STA_time_series_pose_v8c\N=" + string(N) + "\";
...
function algorithm_containerPT = Perturbation_Theory_processing(algorithm_containerPT)
    ...
    [vectors_final, lambda_final, inertiaTensor_final, ~] = iterative_perturbation(vectors_start, lambda_start, lambda_target);
    
    algorithm_containerPT.vectors_final = vectors_final;
    algorithm_containerPT.inertiaTensor_final = inertiaTensor_final;
    algorithm_containerPT.eigenvalues_final = lambda_final;   

end

function algorithm_containerPT = Perturbation_Theory_reprocessing(algorithm_containerPT)
    ...
    m_star = algorithm_containerPT.mass_redistribution;
    scaling_factor_CM = algorithm_containerPT.scaling_factor_CM;
    
    vectors_final = algorithm_containerPT.vectors_final;
    inertiaTensor_final = algorithm_containerPT.inertiaTensor_final;
    eigenvalues_final = algorithm_containerPT.eigenvalues_final;
    
    cm_reflect_bool = false;
    
    cm_vector_noisy = inertial_properties_noisy.CM.vector;
    [cm_vector_constrained, cm_vector_reflected] = cm_constrain_and_reflect(m_star, mass_distribution_noisy, scaling_factor_CM, inertial_properties_noisy, N);
    ...
    inertial_properties_perturbed = struct;
    inertial_properties_perturbed = compute_inertial_properties_constrained(cm_vector_constrained, inertial_properties_perturbed);
    ...
    new_rotationMatrix = compute_rotation_given_translation(inertial_properties_perturbed.T, N, mass_distribution_noisy, mass_distribution_reference);
    
    inertial_properties_perturbed = add_inertial_properties_perturbation(new_rotationMatrix, eigenvalues_final, inertiaTensor_final, inertial_properties_perturbed);
...
function output_containers = generate_output_containers(input_size)
    ...
    for n=1:NumAlgorithms
        ...
        inertiaTensor.(algorithm_str_current) = nan(NumFrames, 3, 3);
        noisy_positions.(algorithm_str_current) = nan(NumFrames, N, 3);
    end
...
function H = nLsysMDall(m, output_containers, input_size)
    
    N = input_size.N;
    NumFrames = input_size.NumFrames;
    for j=1:NumFrames
        inertiaTensor_final = reshape(output_containers.inertiaTensor.PTNR(j, :, :), [3,3]);
        ...
        for i=1:N
            current_noisy_position = reshape(output_containers.noisy_positions.PTNR(j, i, :), [3,1]);
            x_sum = x_sum + m(i)*current_noisy_position(1);
            y_sum = y_sum + m(i)*current_noisy_position(2);
            z_sum = z_sum + m(i)*current_noisy_position(3);
            m_sum = m_sum + m(i);
        end
        x_cm = x_sum/m_sum;
        y_cm = y_sum/m_sum;
        ...
        for i=1:N
            current_noisy_position = reshape(output_containers.noisy_positions.PTNR(j, i, :), [3,1]);
            x_rel = current_noisy_position(1) - x_cm;
            y_rel = current_noisy_position(2) - y_cm;
            z_rel = current_noisy_position(3) - z_cm;
            sumVal(1) = sumVal(1) + m(i)*(y_rel^2 + z_rel^2);
            sumVal(2) = sumVal(2) - m(i)*x_rel*y_rel;
            sumVal(3) = sumVal(3) - m(i)*x_rel*z_rel;
            sumVal(4) = sumVal(4) + m(i)*(x_rel^2 + z_rel^2);
        ...
        k = 6*(j-1);
        H(k+1) = sumVal(1) - inertiaTensor_final(1,1);
        H(k+2) = sumVal(2) - inertiaTensor_final(1,2);
        H(k+3) = sumVal(3) - inertiaTensor_final(1,3);
        H(k+4) = sumVal(4) - inertiaTensor_final(2,2);
        H(k+5) = sumVal(5) - inertiaTensor_final(2,3);
        H(k+6) = sumVal(6) - inertiaTensor_final(3,3);
    end
...
function [vector_cm_constrained, vector_cm_reflected] = cm_constrain_and_reflect(m, mass_distribution, scaling_factor_CM, inertial_properties, N)
    x_sum = 0;
    y_sum = 0;
    z_sum = 0;
    m_sum = 0;
    ...
    y_cm = y_sum/m_sum;
    z_cm = z_sum/m_sum;
    vector_cm_unconstrained = [x_cm; y_cm; z_cm];
    vector_cm_noisy = inertial_properties.CM.vector;
    vector_cm_constrained = (1-scaling_factor_CM)*vector_cm_noisy + scaling_factor_CM*vector_cm_unconstrained;
    vector_cm_relative = vector_cm_constrained - inertial_properties.CM.vector;
    vector_cm_reflected = vector_cm_noisy - vector_cm_relative;
end

function inertial_properties_constrained = compute_inertial_properties_constrained(cm_constrained_vector, inertial_properties)
    
    inertial_properties_constrained = inertial_properties;
    
    inertial_properties_constrained.CM.x = cm_constrained_vector(1);
    inertial_properties_constrained.CM.y = cm_constrained_vector(2);
    inertial_properties_constrained.CM.z = cm_constrained_vector(3);
    inertial_properties_constrained.CM.vector = [cm_constrained_vector(1); cm_constrained_vector(2); cm_constrained_vector(3)];
    inertial_properties_constrained.T = inertial_properties_constrained.CM.vector;
...
function inertial_properties_added = add_inertial_properties_perturbation(new_rotationMatrix, eigenvalues_final, inertiaTensor_final, inertial_properties_old)
    ...
    inertial_properties_added.PrincipalAxis(1).vector = new_rotationMatrix(:,1);
    inertial_properties_added.PrincipalAxis(2).vector = new_rotationMatrix(:,2);
    inertial_properties_added.PrincipalAxis(3).vector = new_rotationMatrix(:,3);
    inertial_properties_added.R = new_rotationMatrix;
    
    inertial_properties_added.PrincipalMOI = struct;
    inertial_properties_added.PrincipalMOI(1).value = eigenvalues_final(1);
    inertial_properties_added.PrincipalMOI(2).value = eigenvalues_final(2);
    inertial_properties_added.PrincipalMOI(3).value = eigenvalues_final(3);
    inertial_properties_added.PrincipalMOINorm = norm(eigenvalues_final);
...
function create_animation_RF(input_struct, output_containers, reference_frame_str)
    ...
    path_img = "C:\Users\vkarmarkar\OneDrive - University of Iowa\Desktop\Research\Code\Test Code\STA_time_series_pose_v8c\N=" + string(input_struct.input_size.N) + "\";
